(function () {
  ...
  const projectName = 'Max Flow Network Lab';
  const downloadSlug = 'max-flow-network-lab';
  const metricLabel = 'Flow';
  ...
  function draw(result) {
    ...
    links.forEach((link) => {
      ...
      if (!a || !b) return;
      const strength = Math.max(0.16, Math.min(1, Number(link[2]) || 0.2));
      context.strokeStyle = 'rgba(125,211,252,' + (0.16 + strength * 0.54).toFixed(3) + ')';
      context.lineWidth = 1.4 + strength * 4;
      context.beginPath();
    ...
    points.forEach((point, index) => {
      const active = pathSet.has(index) || point.r > 4.6;
    ...
    if (series.length > 0) {
      ...
      series.forEach((value, index) => {
        ...
        const height = normalized * canvas.height * 0.34;
        context.fillStyle = value >= 0 ? 'rgba(247,223,30,.66)' : 'rgba(248,113,113,.66)';
