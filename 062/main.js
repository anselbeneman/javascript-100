(function () {
  ...
  document.getElementById('pngButton').addEventListener('click', () => download('reservoir-sampling.png', canvas.toDataURL('image/png')));
  document.getElementById('jsonButton').addEventListener('click', () => {
    ...
    download('reservoir-sampling.json', URL.createObjectURL(new Blob([payload], { type: 'application/json' })));
  });
