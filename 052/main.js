(function () {
  ...
  document.getElementById('pngButton').addEventListener('click', () => download('fenwick-range-sum.png', canvas.toDataURL('image/png')));
  document.getElementById('jsonButton').addEventListener('click', () => {
    ...
    download('fenwick-range-sum.json', URL.createObjectURL(new Blob([payload], { type: 'application/json' })));
  });
  document.getElementById('reportButton').addEventListener('click', async () => {
    await navigator.clipboard.writeText('Fenwick Range Sum Lab: ' + JSON.stringify(latest.metrics));
