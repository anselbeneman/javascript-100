(function () {
  ...
  document.getElementById('pngButton').addEventListener('click', () => download('kd-tree-nearest.png', canvas.toDataURL('image/png')));
  document.getElementById('jsonButton').addEventListener('click', () => {
    ...
    download('kd-tree-nearest.json', URL.createObjectURL(new Blob([payload], { type: 'application/json' })));
  });
  document.getElementById('reportButton').addEventListener('click', async () => {
    await navigator.clipboard.writeText('KD-Tree Nearest Neighbor: ' + JSON.stringify(latest.metrics));
