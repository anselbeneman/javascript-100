(function () {
  ...
  document.getElementById('pngButton').addEventListener('click', () => download('tarjan-scc-explorer.png', canvas.toDataURL('image/png')));
  document.getElementById('jsonButton').addEventListener('click', () => {
    ...
    download('tarjan-scc-explorer.json', URL.createObjectURL(new Blob([payload], { type: 'application/json' })));
  });
  document.getElementById('reportButton').addEventListener('click', async () => {
    await navigator.clipboard.writeText('Tarjan SCC Explorer: ' + JSON.stringify(latest.metrics));
