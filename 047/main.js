(function () {
  ...
  document.getElementById('pngButton').addEventListener('click', () => download('trie-autocomplete.png', canvas.toDataURL('image/png')));
  document.getElementById('jsonButton').addEventListener('click', () => {
    ...
    download('trie-autocomplete.json', URL.createObjectURL(new Blob([payload], { type: 'application/json' })));
  });
  document.getElementById('reportButton').addEventListener('click', async () => {
    await navigator.clipboard.writeText('Trie Autocomplete Index: ' + JSON.stringify(latest.metrics));
